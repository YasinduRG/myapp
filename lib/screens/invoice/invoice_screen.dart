...
class _InvoiceScreenState extends State<InvoiceScreen> {
  ...
  final List<Dealer> _dealers = [
    Dealer(name: 'Containers Co.', surname: 'Containers', accountCode: 'AC2000123230', address: 'Test Address 1', city: 'City 1'),
    Dealer(name: 'B Motors', surname: 'B Motors', accountCode: 'AC2000123231', address: 'Test Address 2', city: 'City 2'),
    Dealer(name: 'General Supplies', surname: 'Supplies', accountCode: 'AC2000123456', address: 'Main Street 123', city: 'City 1'),
    Dealer(name: 'Auto Parts', surname: 'Auto', accountCode: 'AC2000123789', address: 'Industrial Ave', city: 'City 3'),
  ];
  ...
  Widget build(BuildContext context) {
    ...
    switch (_currentStep) {
      case 0:
        ...
        break;
      case 1:
        currentView = AuthenticateDealerView(dealer: _selectedDealer!, onAuthenticated: _onAuthenticated);
        break;
      case 2:
        ...
        break;
      case 3:
        currentView = CreateInvoiceView(dealer: _selectedDealer!, tindata: _selectedTin!);
        break;
      default:
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      _currentStep == 0 ? 'Select Dealer' : _currentStep == 1 ? 'Authenticate Dealer' : 'Invoice',
...
class _SelectDealerViewState extends State<SelectDealerView> {
  ...
  void _filterDealers() {
    ...
    setState(() {
      _filteredDealers = widget.dealers.where((dealer) {
      ...
      builder: (_) {
        return SelectionSheet<Dealer>(
          ...
          headerBuilder: const Row(
            ...
            children: [
              Expanded(child: Text('Account Code', style: TextStyle(fontWeight: FontWeight.bold))),
              Expanded(child: Text('Surname', style: TextStyle(fontWeight: FontWeight.bold))),
              Expanded(child: Text('Address', style: TextStyle(fontWeight: FontWeight.bold))),
              Expanded(child: Text('City', style: TextStyle(fontWeight: FontWeight.bold))),
            ],
          ),
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const Spacer(),
          ElevatedButton.icon(
            onPressed: widget.selectedDealer != null ? widget.onSubmit : null,
...
class _SelectTinNumberViewState extends State<SelectTinNumberView> {
  ...
  void _filterTins() {
    final query = _searchController.text.toLowerCase();
    setState(() {
      _filteredTins = widget.tins.where((tin) {
      ...
      builder: (_) {
        ...
        return SelectionSheet<TinData>(
          ...
          headerBuilder: const Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Expanded(child: Text('TIN Number', style: TextStyle(fontWeight: FontWeight.bold))),
              Expanded(child: Text('Total Value', style: TextStyle(fontWeight: FontWeight.bold))),
            ],
          ),
              ...
              child: Padding(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Expanded(child: Text(tin.totalValue.toStringAsFixed(2))), // Format value for display
                  ],
                ),
              ),
            );
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
            ...
            child: Text('${widget.dealer.name} - ${widget.dealer.accountCode}', style: const TextStyle(fontWeight: FontWeight.bold)),
          ),
          ...
          // The submit button.
          ElevatedButton.icon(
            onPressed: widget.selectedTin != null ? widget.onSubmit : null,
            icon: const Icon(Icons.check_circle_outline, color: Colors.white),
            label: const Text('Submit', style: TextStyle(color: Colors.white, fontSize: 16)),
            style: ElevatedButton.styleFrom(
              backgroundColor: AppColors.primary,
...
class CreateInvoiceView extends StatefulWidget {
  ...
  const CreateInvoiceView({super.key, required this.dealer, required this.tindata});
...
class _CreateInvoiceViewState extends State<CreateInvoiceView> {
  ...
  Widget build(BuildContext context) {
    ...
    if (_searchQuery.isEmpty) {
      ...
      filteredParts = _parts.where((part) {
        // Check if the part number (in lowercase) contains the search query (in lowercase).
        return part.partNo.toLowerCase().contains(_searchQuery.toLowerCase());
      }).toList();
    }
    return Padding(
  ...
  Widget _buildFilterAndSearch() {
    return Container(
      ...
      child: Row(
        children: [
          TextButton.icon(onPressed: () {}, icon: const Icon(Icons.filter_alt_outlined), label: const Text('Filter')),
          const VerticalDivider(width: 1, indent: 8, endIndent: 8), // A thin visible divider
          Expanded(
            child: TextField(
    ...
    return Column(
      children: [
        Container(
          ...
          padding: const EdgeInsets.all(12),
          decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(8)),
          child: Text('${dealer.name} - ${dealer.accountCode}', style: const TextStyle(fontWeight: FontWeight.bold)),
        ),
        const SizedBox(height: 16),
        Row(
          children: [
              ...
              child: Container(
                ...
                child: DropdownButtonHideUnderline(
                  child: DropdownButton<String>(
                    ...
                    items: [tindata.tinNumber].map((String value) {
  ...
  Widget _buildPartsTableHeader() {
    return Container(
      ...
      child: const Row(
        children: [
          Expanded(flex: 3, child: Text('Part No', style: TextStyle(fontWeight: FontWeight.bold))),
          Expanded(flex: 2, child: Text('Request Qty', style: TextStyle(fontWeight: FontWeight.bold))),
          Expanded(flex: 2, child: Text('Select', style: TextStyle(fontWeight: FontWeight.bold))),
          Expanded(flex: 3, child: Text('Receive Qty', style: TextStyle(fontWeight: FontWeight.bold))),
        ],
      ),
    );
  }
  ...
  Widget _buildSubmitButton() {
     return ElevatedButton.icon(
      ...
      onPressed: () {
          // Add save logic here and send to the back end
...
class _QuantityStepper extends StatelessWidget {
  ...
  const _QuantityStepper({required this.quantity, required this.enabled, required this.onTap});
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            ...
            Text(quantity.toString(), style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: color)),
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      content: Row(
        ...
        children: [
          IconButton(
            icon: const Icon(Icons.remove_circle, color: AppColors.danger, size: 30),
            onPressed: () {
          ...
          Text(_currentQuantity.toString(), style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
          IconButton(
      ...
      actions: [
        ...
        ElevatedButton(
          onPressed: () => Navigator.of(context).pop(_currentQuantity), // Pop with the new quantity
